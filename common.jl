...
function check_answer(solution::Int, day_number::Int, part_number::Int)::Nothing
   
    fname = "output/output" * string(day_number) * "_" * string(part_number) * ".txt"

    check =  open(fname) do file
        parse(Int, readline(file))
    end

    evaluation_str = if solution == check "correct!" else "wrong!" end
